  ...
  if (num_sites % 2 != 0)
    ...
    for j = 1:2:num_sites-1
      ...
      elseif (j == num_sites-1)
        ...
        for i in 1:4
          A[s1=>i, s2=>(5-i), rightlink=>1, leftlink => 1] = 1/2
        end
...
# 4-dimensional coarsegrained Hilbert space
function ITensors.space(::SiteType"SU(2)";
  conserve_qns=false)
  # if conserve_qns
    # return [
    #   QN(("Q1", 1), ("Q2", 0)) => 1
...
ITensors.state(::StateName"11", ::SiteType"SU(2)") = [1.0, 0, 0, 0]
ITensors.state(::StateName"12", ::SiteType"SU(2)") = [0, 1.0, 0, 0]
ITensors.state(::StateName"21", ::SiteType"SU(2)") = [0, 0, 1.0, 0]
ITensors.state(::StateName"22", ::SiteType"SU(2)") = [0, 0, 0, 1.0]
...
ITensors.op(::OpName"S1z",::SiteType"SU(2)") =
kron(Sz, Id)

ITensors.op(::OpName"S2z",::SiteType"SU(2)") =
kron(Id, Sz)
...
for i in 1:N_hilbert
  for j in 1:N_hilbert
    ...
    @eval begin
      ITensors.op(::OpName{$(QuoteNode(op_name))}, ::SiteType"SU(2)") =
...
function main(params::SimulationParameters)
  ...
  # filename = "/pscratch/sd/k/kwang98/KPZ/tdvp_su(2)_dw_gpu_L$(params.L)_chi$(params.maxdim)_beta$(params.β_max)_dt$(params.δt)_U$(params.U)_mu$(params.μ).h5"
  # filename = "/global/scratch/users/kwang98/KPZ/tdvp_su(2)_dw_gpu_L$(params.L)_chi$(params.maxdim)_beta$(params.β_max)_dt$(params.δt)_U$(params.U)_mu$(params.μ).h5"
  filename = "tdvp_su(2)_dw_gpu_L$(params.L)_chi$(params.maxdim)_beta$(params.β_max)_dt$(params.δt)_U$(params.U)_mu$(params.μ).h5"
  ...
  if (isfile(filename))
    ...
    sites = siteinds("SU(2)", 2 * params.L; conserve_qns=false)
