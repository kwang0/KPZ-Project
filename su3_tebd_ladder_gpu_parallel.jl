...
function find_gammas(ψ)
  N = length(ψ)
  Γs = ITensor[]

  orthogonalize!(ψ, 1)
  U,S,V = svd(ψ[1], (siteinds(ψ, 1)...))
  push!(Γs, cu(S))
  for i in 2:N
    orthogonalize!(ψ, i)
    U,S,V = svd(ψ[i], (linkinds(ψ, i-1)..., siteinds(ψ, i)...))
    push!(Γs, cu(S))
  end
...
function updateblock(ψ, i, Γs, W1, W2, cut, m, alg)
  ...
  Φ_bar = apply(W1, block)
  Φ_bar = apply(W2, block)

  if (i == 1)
    Φ = Φ_bar

    U,S,V = svd(Φ, (sites[i], sites[i+1], sites[i+2]), cutoff=cut, maxdim=m, alg=alg, righttags="Link,l=$(i+2)")
    ψ[i+3] = V
    Γs[i+2] = S
    Φ = U*S
    Φ_bar = Φ_bar * dag(V)

    U,S,V = svd(Φ, (sites[i], sites[i+1]), cutoff=cut, maxdim=m, alg=alg, righttags="Link,l=$(i+1)")
    ψ[i+2] = V
    Γs[i+1] = S
    Φ = U*S
    Φ_bar = Φ_bar * dag(V)

    U,S,V = svd(Φ, (sites[i]), cutoff=cut, maxdim=m, alg=alg, righttags="Link,l=$(i)")
    ψ[i+1] = V
    Γs[i] = S
    ψ[i] = Φ_bar * dag(V)
  else
    ...
    U,S,V = svd(Φ, (sites[i], sites[i+1], sites[i+2], leftlink), cutoff=cut, maxdim=m, alg=alg, righttags="Link,l=$(i+2)")
    ψ[i+3] = V
    Γs[i+2] = S
    Φ = U*S
    Φ_bar = Φ_bar * dag(V)

    U,S,V = svd(Φ, (sites[i], sites[i+1], leftlink), cutoff=cut, maxdim=m, alg=alg, righttags="Link,l=$(i+1)")
    ψ[i+2] = V
    Γs[i+1] = S
    Φ = U*S
    Φ_bar = Φ_bar * dag(V)

    U,S,V = svd(Φ, (sites[i], leftlink), cutoff=cut, maxdim=m, alg=alg, righttags="Link,l=$(i)")
    ψ[i+1] = V
    Γs[i] = S
    ψ[i] = Φ_bar * dag(V)
  end
