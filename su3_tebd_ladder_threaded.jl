...
function find_lambdas(ψ)
  ...
  ϕ = orthogonalize(ψ, 1)
  U,S,V = svd(ϕ[1], (siteinds(ϕ, 1)...))
  leftind = commonind(U,S)
  rightind = commonind(S,V)
  linkind = commonind(ψ[1], ψ[2])
  replaceinds!(S, [leftind, rightind], [linkind', linkind]) # Priming the left side of Lambda, same index as right
  push!(Λs, S)
  for i in 2:(N-1)
    orthogonalize!(ϕ, i)
    U,S,V = svd(ϕ[i], (linkinds(ϕ, i-1)..., siteinds(ϕ, i)...))
    leftind = commonind(U,S)
    rightind = commonind(S,V)
    linkind = commonind(ψ[i], ψ[i+1])
    replaceinds!(S, [leftind, rightind], [linkind', linkind])
    push!(Λs, S)
  end
...
function updateblock(ψ, sites, i, Λs, W1, W2, cut, m)
  ...
  U,S,V = svd(Φ, (sites[i+2], leftlink), cutoff=cut, maxdim=m, righttags="Link,l=$(i+1)")
  B3 = V
  B2 = Φ_bar * dag(V)
  leftind = commonind(U,S)
  rightind = commonind(S,V)
  ...
  if (i == 1)
    ...
    U,S,V = svd(Φ, (sites[i+1], leftlink), cutoff=cut, maxdim=m, righttags="Link,l=$(i+2)")
    ψ[i+3] = V
    B3 = Φ_bar * dag(V)
    leftind = commonind(U,S)
    rightind = commonind(S,V)
  ...
  Φ = noprime(Λs[i] * Φ_bar)
  leftlink = dag(commonind(ψ[i], Φ))
  U,S,V = svd(Φ, (sites[i+1], leftlink), cutoff=cut, maxdim=m, righttags="Link,l=$(i+1)")
  ψ[i+2] = V
  ψ[i+1] = Φ_bar * dag(V)
  leftind = commonind(U,S)
  rightind = commonind(S,V)
...
function main(params::SimulationParameters)
  ...
  filename = "/pscratch/sd/k/kwang98/KPZ/tebd_su(3)_dw_L$(params.L)_chi$(params.maxdim)_beta$(params.β_max)_dt$(params.δt)_U$(params.U)_mu$(params.μ)_threaded.h5"
