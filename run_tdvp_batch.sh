...
do
    srun --exact -u -n 1 --gpus-per-task 1 -c 32 --mem-per-gpu=55G julia heisenberg_ladder_tdvp_coarsegrained_dw_gpu.jl 64 600 0.0 0.1 $i 0.0 0.0 0.001 > logs_jl/tdvp_coarsegrained_dw_gpu_L64_chi600_beta0.0_dt0.1_Jprime${i}_U0.0_Uprime0.0_mu0.001.txt &
done
...
for i in $(seq 0.4 0.4 2.0)
do
    srun --exact -u -n 1 --gpus-per-task 1 -c 32 --mem-per-gpu=55G julia heisenberg_ladder_tdvp_coarsegrained_dw_gpu.jl 64 600 0.0 0.1 0.0 0.0 $i 0.001 > logs_jl/tdvp_coarsegrained_dw_gpu_L64_chi600_beta0.0_dt0.1_Jprime0.0_U0.0_Uprime${i}_mu0.001.txt &
done
