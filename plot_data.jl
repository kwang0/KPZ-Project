...
# Plot my own data
function plot_hdf(ax, f::String; norm::Float64=1.0, type = "hdf", graph="twosite", t_scale=1.0, s_scale = 1.0, label="default", dw="Z", ncol=1, window_size = 20, T_cutoff=Inf64, normalize=false, marker="")
    if label == "default"
        label = f
    end
    if graph == "both"
    ...
    elseif graph == "both_transfer"
        plot_hdf(ax[1], f, type=type, graph = "transfer", t_scale=1.0, s_scale=s_scale, label = label, dw = dw, ncol=ncol, window_size=window_size, T_cutoff=T_cutoff, normalize=normalize, marker=marker)
    ...
    elseif graph == "transfer"
        ...
        ax.loglog(times .* t_scale, s_scale .* transfer[:] .* (times .^ (-2/3)), label = label, marker=marker)
        # plt.plot(log.(times), diffusion, label=f)
        # ax.legend(ncol=ncol)
